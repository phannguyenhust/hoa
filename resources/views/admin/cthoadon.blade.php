...
<body class="sb-nav-fixed">
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <tr dir-paginate = "ls in CTHoaDon| filter: timkiem|itemsPerPage:10">
                <td>@{{ $index+1 }}</td>
                <td>@{{ ls.mahd }}</td>
                <td>@{{ ls.masp }}</td>
                <td>@{{ ls.soluong }}</td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                      ...
                      <input type="text"
                        class="form-control" id="mahd" ng-model="cthoadon.mahd">
                    </div>
<div class="form-group">
                        <label for="masp">Mã sản phẩm</label>
                        <input type="text"
                          class="form-control" id="masp" ng-model="cthoadon.masp">
                      </div>
                      <div class="form-group">
                      <label for="soluong">Số lượng</label>
                      <input type="text"
                        class="form-control" id="soluong" ng-model="cthoadon.soluong">
                    </div>
                    <div class="form-group">
                      <label for="gianhap">Giá nhập</label>
                      <input type="text"
                        class="form-control" id="gianhap" ng-model="cthoadon.gianhap">
                    </div>
                </div>
    ...
    <script>
        ...
        app.controller('mycontroller',function($scope,$http){
            ...
            $scope.reloadData = function(){
                    ...
                    {
                        method: "GET",
                        url: "http://localhost:8000/api/CTHoaDon"
                    }
                ).then(function(response){
                    $scope.CTHoaDon = response.data;
                });
            }
            $scope.reloadData();
            ...
            $scope.showModal = function(id){
                $scope.id=id;
                if(id==0){
                    $scope.modalTitle = "Them ct hoa don";
                    if($scope.cthoadon) delete $scope.cthoadon;
                }
                else{
                    $scope.modalTitle = "Sua ct hoa don";
                    $http(
                        {
                            method: "GET",
                            url: "http://localhost:8000/api/CTHoaDon/"+id
                        }
                    ).then(function(response){
            ...
            $scope.saveData = function(){
                if($scope.id==0){
                    ...
                    $http(
                        {
                            method: "PUT",
                            url: "http://localhost:8000/api/CTHoaDon/"+$scope.id,
                            data: $scope.cthoadon,
                            "content-Type": "application/json",
                        }
                    ).then(function(response){

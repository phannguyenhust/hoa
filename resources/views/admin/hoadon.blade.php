...
<body class="sb-nav-fixed">
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <tr dir-paginate = "ls in HoaDon| filter: timkiem|itemsPerPage:10">
                <td>@{{ $index+1 }}</td>
                <td>@{{ ls.mancc }}</td>
                <td>@{{ ls.ngaynhap }}</td>
                <td>@{{ ls.thanhtien }}</td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                      ...
                      <input type="text"
                        class="form-control" id="mancc" ng-model="hoadon.mancc">
                    </div>
<div class="form-group">
                        <label for="ngaynhap">Ngày nhập</label>
                        <input type="text"
                          class="form-control" id="ngaynhap" ng-model="hoadon.ngaynhap">
                      </div>
                <div class="form-group">
                        <label for="thanhtien">Thành tiền</label>
                        <input type="text"
                          class="form-control" id="thanhtien" ng-model="hoadon.thanhtien">
                </div>
                <div class="form-group">
                        <label for="manv">Mã nhân viên</label>
                        <input type="text"
                          class="form-control" id="manv" ng-model="hoadon.manv">
                      </div>
                </div>
    ...
    <script>
        ...
        app.controller('mycontroller',function($scope,$http){
            ...
            $scope.reloadData = function(){
                    ...
                    {
                        method: "GET",
                        url: "http://localhost:8000/api/HoaDon"
                    }
            ...
            $scope.showModal = function(id){
                $scope.id=id;
                if(id==0){
                    $scope.modalTitle = "Them hoa don";
                    if($scope.hoadon) delete $scope.hoadon;
                }
                else{
                    $scope.modalTitle = "Sua hoa don";
                    $http(
                        {
                            method: "GET",
                            url: "http://localhost:8000/api/HoaDon/"+id
                        }
                    ).then(function(response){
            ...
            $scope.saveData = function(){
                if($scope.id==0){
                    ...
                    $http(
                        {
                            method: "PUT",
                            url: "http://localhost:8000/api/HoaDon/"+$scope.id,
                            data: $scope.hoadon,
                            "content-Type": "application/json",
                        }
                    ).then(function(response){
